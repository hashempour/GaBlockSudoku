    ...
    utils: {
        ...
        createPopulation: () => {
            for (let i = 0; i < GENETICS.POPULATION_COUNT; i++) {
                GENETICS.population.push(new Chromosome(GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA(), GENETICS.utils.getRandDNA()));
        ...
        buildNextGeneration: () => {
            ...
            GENETICS.populationGameScore.forEach((score, index) => {
            ...
            // reborn process
            const forRebornIndexes = [];
            const forParentingIndexes = [];
            GENETICS.populationGameScore.forEach((score, index) => {
            ...
            GENETICS.populationGameScore.forEach((score, index) => {
                ...
                if (GENETICS.populationAvgSuccessRates[index].ageCount >
                    GENETICS.REBORN_GEN_DATA.GEN_LIMIT &&
                    GENETICS.populationAvgSuccessRates[index].successRate >
                        GENETICS.avgTotalSuccessRate.worst * 1.2) {
            ...
            GENETICS.populationAvgSuccessRates.forEach((rate, index) => {
                if (rate.successRate > mostSuccessfulChromosomeRate ||
            ...
            if (forParentingIndexes.length > 0 && forRebornIndexes.length > 0) {
                ...
                GENETICS.population[bestChromosome.index], GENETICS.population[bestChromosome.index], true, // mutation enabled
            ...
            if (forParentingIndexes.length > 2 && forRebornIndexes.length > 0) {
                ...
                forRebornIndexes.forEach((rebornIndex) => {
                    ...
                    GENETICS.population[rebornIndex] = GENETICS.utils.getNewChild(GENETICS.population[forParentingIndexes[Math.floor(Math.random() * forParentingIndexes.length)]], GENETICS.population[forParentingIndexes[Math.floor(Math.random() * forParentingIndexes.length)]], true // mutation enabled
                    );
        ...
        getOneCutChromosome: (chromosome1, chromosome2) => {
            const randPosition = Math.floor(Math.random() * (GENETICS.CHROMOSOME_DNA_COUNT - 1)) + 1;
        ...
        getNewChild: (chromosome1, chromosome2, mutation, forceToMutate = false) => {
            let mutate = mutation && (Math.random() < GENETICS.MUTATION_RATE || forceToMutate);
            ...
            return new Chromosome(mutationDnaIndex === 0
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.a
                    : chromosome2.a, mutationDnaIndex === 1
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.b
                    : chromosome2.b, mutationDnaIndex === 2
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.c
                    : chromosome2.c, mutationDnaIndex === 3
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.d
                    : chromosome2.d, mutationDnaIndex === 4
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.e
                    : chromosome2.e, mutationDnaIndex === 5
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.f
                    : chromosome2.f, mutationDnaIndex === 6
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.g
                    : chromosome2.g, mutationDnaIndex === 7
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.h
                    : chromosome2.h, mutationDnaIndex === 8
                ? GENETICS.utils.getRandDNA()
                : Math.random() < 0.5
                    ? chromosome1.i
                    : chromosome2.i);
...
function playNextPopulation() {
    ...
    if (GENETICS.currentPopulation < GENETICS.POPULATION_COUNT) {
        ...
        let bestAvgIndex = -1;
        GENETICS.populationAvgSuccessRates.forEach((avg, index) => {
            if (bestAvgIndex === -1 ||
        ...
        if (VISUALISE !== VISUALISATION_STATE.NONE) {
            ...
            $("#spanBestChromosome").text(GENETICS.population[bestAvgIndex].toString());
            $("#spanBestAvgSuccess").text(GENETICS.populationAvgSuccessRates[bestAvgIndex].successRate * 100);
        }
        GENETICS.currentPopulation = -1;
        playNextPopulation();
    }
...
function playARound(chromosome, elements) {
    ...
    if (bestPracticeOrder.sumValue !== null) {
        ...
        bestPracticeOrder.bestOrderArray.forEach((element, index) => {
            if (DEBUG_MODE == DEBUGMODE_STATE.DEBUG) {
...
function getCopyOfBoardState(boardState = undefined) {
