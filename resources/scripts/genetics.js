...
const GENETICS = {
    ...
    CROSS_OVER_RATE: .25,
    MUTATION_RATE: .4,
    REBORN_GEN_DATA: { GEN_LIMIT: 15, CROSS_OVER_RATE: .10, CROSS_OVER_ELITE_RATE: .30 },
    ...
    utils: {
        getRandDNA: function() {
            // -10.00 .. 0.00 .. +10.00
            return ( Math.floor( Math.random() * GENETICS.CHROMOSOME_DNA_RANGE ) - Math.floor( GENETICS.CHROMOSOME_DNA_RANGE / 2 ) ) / 100;
        },
        createPopulation: function() {
            for ( let i = 0; i < GENETICS.POPULATION_COUNT; i++ ) {
                ...
                GENETICS.populationAvgSuccessRates.push( { s: .0, c: 0 } );  //  { s: successRate, c: ageCount }
                GENETICS.avgTotalSuccessRate = { best: .0, worst: .0, count: 0 };
            }
        },
        buildNextGeneration: function() {
            ...
            GENETICS.populationGameScore.forEach( function( score, index ) {
                ...
                GENETICS.populationAvgSuccessRates[ index ].s =
                    ( GENETICS.populationAvgSuccessRates[ index ].s * GENETICS.populationAvgSuccessRates[ index ].c + ( score / bestChromosome.score ) ) / ( GENETICS.populationAvgSuccessRates[ index ].c + 1 );
            ...
            GENETICS.avgTotalSuccessRate.worst = ( GENETICS.avgTotalSuccessRate.worst * GENETICS.avgTotalSuccessRate.count + GENETICS.populationAvgSuccessRates[ worstChromosome.index ].s ) / ( GENETICS.avgTotalSuccessRate.count + 1 );
            GENETICS.avgTotalSuccessRate.count++;

            GENETICS.populationGameScore.forEach( function( score, index ) {
                // choose unsuccessful population for reborn
                if ( Math.random() < GENETICS.REBORN_GEN_DATA.CROSS_OVER_RATE   // last chance to keep it alive randomly!
                    && GENETICS.populationAvgSuccessRates[ index ].c > GENETICS.REBORN_GEN_DATA.GEN_LIMIT
                    && GENETICS.populationAvgSuccessRates[ index ].s < GENETICS.avgTotalSuccessRate.worst * 1.2 ) {
                    forRebornIndexes.push( index );
                }
                // choose rest population for parent
                if ( GENETICS.populationAvgSuccessRates[ index ].c > GENETICS.REBORN_GEN_DATA.GEN_LIMIT
                    && GENETICS.populationAvgSuccessRates[ index ].s > GENETICS.avgTotalSuccessRate.worst * 1.2 ) {
            ...
            if ( forParentingIndexes.length > 1 && forRebornIndexes.length > 0 ) {
                forRebornIndexes.forEach( function ( rebornIndex ) {
                    // REBORN
                    console.info( 'reborn #' + rebornIndex, GENETICS.populationAvgSuccessRates[ rebornIndex ] );
                    GENETICS.population[ rebornIndex ] = GENETICS.utils.getNewChild (
                        GENETICS.population[ forParentingIndexes[ Math.floor( Math.random() * forParentingIndexes.length ) ] ],
                        GENETICS.population[ forParentingIndexes[ Math.floor( Math.random() * forParentingIndexes.length ) ] ],
                        true    // mutation enabled
                    );

                    // reset avg probability data for new chromosomes
                    GENETICS.populationAvgSuccessRates[ rebornIndex ] = { s: .0, c: 0 };
                } );
            }
        ...
        getOneCutChromosome: function( chromosome1, chromosome2 ) {
            let randPosition = Math.floor( Math.random() * ( GENETICS.CHROMOSOME_DNA_COUNT - 1 ) ) + 1;
        ...
        getNewChild: function( chromosome1, chromosome2, mutation ) {
            let mutationDnaIndex = mutation && Math.random() < GENETICS.MUTATION_RATE
            ...
            return {
                a: mutationDnaIndex === 0
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.a : chromosome2.b,
                b: mutationDnaIndex === 1
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.b : chromosome2.b,
                c: mutationDnaIndex === 2
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.c : chromosome2.c,
                d: mutationDnaIndex === 3
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.d : chromosome2.d,
                e: mutationDnaIndex === 4
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.e : chromosome2.e,
                f: mutationDnaIndex === 5
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.f : chromosome2.f,
                g: mutationDnaIndex === 6
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.g : chromosome2.g,
                h: mutationDnaIndex === 7
                    ? GENETICS.utils.getRandDNA()
                    : Math.random() < .5 ? chromosome1.h : chromosome2.h
            };
        }
    }
};
...
function playNextPopulation() {
    ...
    if ( GENETICS.currentPopulation < GENETICS.POPULATION_COUNT ) {
        ...
        setTimeout( playARound, PLAY_TIME_MS / 2, GENETICS.population[ GENETICS.currentPopulation ] );
    } else {
        // end of population
        ...
        // update some data
        PLAY_INFO.avgScore = Math.round( result.scoreSum / GENETICS.POPULATION_COUNT * 100 ) / 100;
        PLAY_INFO.avgScoreList.push( PLAY_INFO.avgScore );
        if ( VISUALISE > 0 ) {
            $( '#labelGen' ).text( GENETICS.genNum );
            $( '#labelAvgScore' ).text( PLAY_INFO.avgScore );
        }

        console.info( 'AVG / HI SCORE GEN #' + GENETICS.genNum, PLAY_INFO.avgScore, PLAY_INFO.hiScore );

        // find best chromosome for Manual Play
        let bestAvgIndex = null;
        GENETICS.populationAvgSuccessRates.forEach( function( avg, index) {
            if ( bestAvgIndex === null || GENETICS.populationAvgSuccessRates[ bestAvgIndex ].s < avg.s ) bestAvgIndex = index;
        } );
        ...
        if ( VISUALISE > 0 ) {
            ...
            $( '#spanBestChromosome' ).text( flatObjectToString( GENETICS.population[ bestAvgIndex ] ) );
            $( '#spanBestAvgSuccess' ).text( GENETICS.populationAvgSuccessRates[ bestAvgIndex ].s * 100 );
        }
...
function flatObjectToString( flatObject ) {
    ...
    Object.keys( flatObject).forEach( function( key ) {
        resultString += ( resultString.length === 0 ? "{ ": ", " ) + key + ": " + flatObject[ key ];
    });
...
function playARound( chromosome, elements ) {
    ...
    for ( let index = 0; index < FACTORIAL_RESULT_SET[ elements.length ]; index++ ) {   // 3! = 6 ; all possible place orders for 3 elements
        ...
        for ( let elementIndex = 0; elementIndex < pickupElementOrder.length; elementIndex++ ) {
            let bestPractice = {
                value: null,
            ...
            for ( let i = ELEMENT_PATTERN_SIZE * -1; i < MAX; i++ ) {
                for ( let j = ELEMENT_PATTERN_SIZE * -1; j < MAX; j++ ) {
                    if ( isPossibleToDrawOnStage( element, { x: i, y: j }, boardStateSimulation ) ) {
                        // simulate the element in current position
                        let calcValue = Math.abs( getCalculatedValue( element, { x: i, y: j }, chromosome, boardStateSimulation ) );
                        if ( isNaN( calcValue ) ) { // TODO: test for debug
                            //debugger;
        ...
        //debugger;
        if ( currentOrderState.bestPositions.length === 3 ) {  // successful order try
            // check the order-round performance
            if ( bestPracticeOrder.sumValue === null || bestPracticeOrder.sumValue > currentOrderState.sumValue ) {
    ...
    if ( bestPracticeOrder.sumValue !== null ) {   // found an acceptable order
        ...
        bestPracticeOrder.bestOrderArray.forEach( function( element, index ) {
            if ( DEBUG_MODE == 2 ) {
                console.info( 'draw #' + ( index + 1 ), element, bestPracticeOrder.bestPositionArray[ index ] );
            }

            if ( HALT_LEARNING && !LEARN_IN_PROGRESS ) {
                // for better presentation
                setTimeout( function() {
                    drawElementOnStage( element, bestPracticeOrder.bestPositionArray[ index ], 'fresh' );
                    checkForCleanup();
                }, index * 2000 );
    ...
    for ( var i = 0; i < ELEMENT_PATTERN_SIZE; i++ ) {
        for (var j = 0; j < ELEMENT_PATTERN_SIZE; j++) {
            if (element.PATTERN[j * ELEMENT_PATTERN_SIZE + i]) {
                boardState[ ( position.y + j ) * GAME_INFO.BOARD_SIZE_BLOCK + position.x + i ] = 1;
            }
        }
    }
}
